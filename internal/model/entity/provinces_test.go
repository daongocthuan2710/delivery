...
func testProvincesExists(t *testing.T) {
	...
	e, err := ProvinceExists(ctx, tx, o.ID)
	if err != nil {
...
func testProvincesFind(t *testing.T) {
	...
	provinceFound, err := FindProvince(ctx, tx, o.ID)
	if err != nil {
		t.Error(err)
	}
...
var (
	provinceDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `GHNID`: `integer`, `GHNCode`: `text`}
	_               = bytes.MinRead
)
